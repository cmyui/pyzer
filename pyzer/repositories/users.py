...
def create_model(
    ...
    """Create a User model from the required database rows."""
    return models.User(
        id=user_row["id"],
        ...
        statistics=models.UserStatistics(
            ...
            grade_counts={"ss": 0, "ssh": 0, "s": 0, "sh": 0, "a": 0},  # TODO
            rank=models.UserRank(
                global_=15000,
                country=30000,
            ),  # TODO: where is this used?
        ),
        support_level=user_row["support_level"],
...
async def fetch_friends_by_id(id: int) -> list[int]:
    """Fetch a user's friends ids by their id."""
...
async def fetch_by_id(id: int) -> Optional[models.User]:
    ...
    return create_model(user_row, statistics_row)


async def fetch_by_username(username: int) -> Optional[models.User]:
    ...
    statistics_row = await services.database.fetch_one(
        "SELECT * FROM user_statistics WHERE username = :username",
        {"username": username},
    )
    if statistics_row is None:
        return None
